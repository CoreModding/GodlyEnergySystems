...
public abstract class Machine extends TileEntity
{
    ...
    {
        Location loc = new Location(this.xCoord, this.yCoord, this.zCoord, this.worldObj);
        if (loc.getWorld() != null)
        {
            ArrayList<Machine> touching = new ArrayList<>();
            if (loc.getWorld().getTileEntity(loc.getX(), loc.getY(), loc.getZ()) instanceof Machine) touching.add((Machine) loc.getWorld().getTileEntity(loc.getX() + 1, loc.getY(), loc.getZ()));
            if (loc.getWorld().getTileEntity(loc.getX(), loc.getY(), loc.getZ()) instanceof Machine) touching.add((Machine) loc.getWorld().getTileEntity(loc.getX() - 1, loc.getY(), loc.getZ()));
            if (loc.getWorld().getTileEntity(loc.getX(), loc.getY(), loc.getZ()) instanceof Machine) touching.add((Machine) loc.getWorld().getTileEntity(loc.getX(), loc.getY() + 1, loc.getZ()));
            if (loc.getWorld().getTileEntity(loc.getX(), loc.getY(), loc.getZ()) instanceof Machine) touching.add((Machine) loc.getWorld().getTileEntity(loc.getX(), loc.getY() - 1, loc.getZ()));
            if (loc.getWorld().getTileEntity(loc.getX(), loc.getY(), loc.getZ()) instanceof Machine) touching.add((Machine) loc.getWorld().getTileEntity(loc.getX(), loc.getY(), loc.getZ() + 1));
            if (loc.getWorld().getTileEntity(loc.getX(), loc.getY(), loc.getZ()) instanceof Machine) touching.add((Machine) loc.getWorld().getTileEntity(loc.getX(), loc.getY(), loc.getZ() - 1));
            return touching;
        }
        return null;
    ...
    public void readFromNBT(NBTTagCompound nbt)
    {
        try
        {
            this.MachinePacket = (Packet) Serializer.deserialize(nbt.getByteArray("MachinePacket"));
        }
     ...
     */
    public abstract boolean ShouldAccept(Packet.SubPacket packet, Machine machine);
     ...
     */
    public abstract boolean ShouldAttemptToSend(Packet.SubPacket packet, Machine machine);
    ...
    public void updateEntity()
    {
        ...
        {
            Packet.SubPacket[] packets = this.MachinePacket.getSubs().toArray(new Packet.SubPacket[] {});
                ...
                if (!packet.lastAt.equals(machine))
                {
                    if (ShouldAttemptToSend(packet, machine))
                    {
                        if (machine.ShouldAccept(packet, this))
                        {
                            try
                            {
                                machine.MachinePacket.mergePacket(packet, machine);
                            }
    ...
    public void writeToNBT(NBTTagCompound nbt)
    {
        try
        {
            nbt.setByteArray("MachinePacket", Serializer.serialize(this.MachinePacket));
        }
        catch (IOException e)
        {
            e.printStackTrace();
